 ...
 * @subpackage Wp_Initial_Cleanup/admin
 */
 ...
 */
class Wp_Initial_Cleanup_Admin {
	...
	public function __construct( $plugin_name, $version ) {
		...
		$this->wp_initial_cleanup_options = get_option($this->plugin_name);
	}
	...
	public function enqueue_styles() {
		 ...
		 * defined in Wp_Initial_Cleanup_Loader as all of the hooks are defined
		 * in that particular class.
		 *
		 * The Wp_Initial_Cleanup_Loader will then create the relationship
		 * between the defined hooks and the functions defined in this
		 * class.
		 */

	
		if ( 'settings_page_wp-initial-cleanup' == get_current_screen() -> id ) {
             // CSS stylesheet for Color Picker
             wp_enqueue_style( 'wp-color-picker' );            
             wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/wp-initial-cleanup-admin.css', array( 'wp-color-picker' ), $this->version, 'all' );
         }
	...
	public function enqueue_scripts() {
		...
		if ( 'settings_page_wp-initial-cleanup' == get_current_screen() -> id ) {
            wp_enqueue_media();   
            wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/wp-initial-cleanup-admin.js', array( 'jquery', 'wp-color-picker' ), $this->version, false );         
        }
	...
	public function add_plugin_admin_menu() {
		 ...
		 *
		 */
		add_options_page( 'Wordpress Initial Cleanup and Customizing Setup', 'WP Initial Cleanup and Customizing Setup', 'manage_options', $this->plugin_name, array($this, 'display_plugin_setup_page')
		);
	}
	...
	public function display_plugin_setup_page() {
		include_once( 'partials/wp-initial-cleanup-admin-display.php' );
	}
   ...
   // Login Customization
     ...
     * @since    1.0.0
     */
     private function wp_initial_cleanup_login_logo_css(){
         if(isset($this->wp_initial_cleanup_options['login_logo_id']) && !empty($this->wp_initial_cleanup_options['login_logo_id'])){
             $login_logo = wp_get_attachment_image_src($this->wp_initial_cleanup_options['login_logo_id'], 'thumbnail');
     ...
     private function wp_initial_cleanup_login_background_color(){
         if(isset($this->wp_initial_cleanup_options['login_background_color']) && !empty($this->wp_initial_cleanup_options['login_background_color']) ){
             $background_color_css  = "body.login{ background:".$this->wp_initial_cleanup_options['login_background_color']."!important;}";
             return $background_color_css;
         }
     }
     // Get Button and links color is set and different from #00A0D2 return it's css
     private function wp_initial_cleanup_login_button_color(){
         if(isset($this->wp_initial_cleanup_options['login_button_primary_color']) && !empty($this->wp_initial_cleanup_options['login_button_primary_color']) ){
             $button_color = $this->wp_initial_cleanup_options['login_button_primary_color'];
     ...
     public function wp_initial_cleanup_login_css(){
         if( !empty($this->wp_initial_cleanup_options['login_logo_id']) || $this->wp_initial_cleanup_login_background_color() != null || $this->wp_initial_cleanup_login_button_color() != null){
             echo '<style>';
             if( !empty($this->wp_initial_cleanup_options['login_logo_id'])){
                   echo $this->wp_initial_cleanup_login_logo_css();
             }
             if($this->wp_initial_cleanup_login_background_color() != null){
                   echo $this->wp_initial_cleanup_login_background_color();
             }
             if($this->wp_initial_cleanup_login_button_color() != null){
                   echo $this->wp_initial_cleanup_login_button_color();
             }
             echo '</style>';
         }
     }
